...
class LineChart: UIView {
    ...
    private func convertDataEntriesToPoints(entries: [PointEntry]) -> [CGPoint] {
        if var min = entries.min()?.value {
            min = 0
            var result: [CGPoint] = []
            let minMaxRange: CGFloat = 100// CGFloat(max - min) * topHorizontalLine
    ...
    private func drawHorizontalLines() {
        ...
        if let gridValues = gridValues {
            for value in gridValues {
                ...
                var lineValue:Int = 0
                if var min = dataEntries.min()?.value {
                    min = 0
                    minMaxGap = 100// CGFloat(max - min) * topHorizontalLine
